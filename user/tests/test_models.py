...
class TestUserModel(APITestCase):
    def setUp(self):
        ...
        self.user1 = User.objects.create_user(
            username="test1", email="test1@test.com", password="testing123"
        )
        self.user2 = User.objects.create_user(
            username="test2", email="test2@test.com", password="testing123"
        )
        self.user3 = User.objects.create_user(
            username="test3", email="test3@test.com", password="testing123"
        )
    ...
    def test_is_friend_of_method(self):
        self.user1.following.add(self.user)
        self.assertTrue(self.user.is_friend_of(self.user1.id))
        self.assertFalse(self.user.is_friend_of(self.user3.id))
